...
class Admin extends Admin_Controller {
    ...
    function login() {
        ...
        if ($this->input->post('login')) {
            //$this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|max_length[100]|xss_clean'); //codeigniter 3.3.10
            //$this->form_validation->set_rules('password', 'Password', 'trim|required|max_length[25]|xss_clean'); //codeigniter 3.3.10
			$this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|max_length[100]'); //codeigniter 3.3.11
            $this->form_validation->set_rules('password', 'Password', 'trim|required|max_length[25]'); //codeigniter 3.3.11
    ...
    function logout() {
        if ($this->userauth->is_logged_in() && $this->userauth->is_admin()) {
            ...
            //$this->session->sess_create(); //remove this line in codeigniter 3.1.11
